  ...
  useEffect(() => {
    ...
    function animate(timeStamp) {
      ...
      if (gameOver && !scoreSaved) {
        scoreSaved = true;
        const existingHigh = parseInt(localStorage.getItem('highScore') || 0);
        if (score > existingHigh) {
          localStorage.setItem('highScore', score);
        }

        const records = JSON.parse(localStorage.getItem('scoreHistory')) || [];
        if (records.length === 0 || records[0].score !== score) {
          records.unshift({
            score,
            date: new Date().toLocaleDateString(),
            time: new Date().toLocaleTimeString(),
          });
          localStorage.setItem('scoreHistory', JSON.stringify(records));
        }
      }
